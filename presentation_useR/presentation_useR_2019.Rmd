...
\begin{block}{C++ code}
\lstinputlisting[language=C++, firstline=1, lastline=7]{Code/addition.cpp}  
\end{block}
\begin{center}
\begin{tabular}{ccc}
\includegraphics[height=1.3cm, width=1.3cm]{figs/Rcpp-logo.png}  &  \Huge{\MVArrowDown} & \large{compile and execute}
\end{tabular}
\end{center}

#### R code
```{r addition-r, echo =T, include=T}
...
\bcols
\bcol{0.1\textwidth}
\includegraphics[height=1.5cm, width=1.5cm]{figs/GNU-logo.png}
\vspace{0.5cm}
\includegraphics[height=1.5cm, width=1.5cm]{figs/armadillo_logo2.png}  
\vspace{0.5cm}
\includegraphics[height=1.5cm, width=1.5cm]{figs/Rcpp-logo.png} 

\ecol

\bcol{0.9\textwidth}

- **GNU Scientific Library** written in C : fiable random number generators, fast random number ditributions, vectors and matrices.
\vspace{0.5cm}
...
### Benefits of using GSL's randow draws instead of R's 
...
\begin{block}{}
\lstinputlisting[language=C++, firstline=1, lastline=18]{Code/example.cpp}  
\end{block}

### Rcpp function using RcppGSL and RcppArmadillo
\begin{block}{}
\lstinputlisting[language=C++, firstline=19, lastline=31, firstnumber=19]{Code/example.cpp}  
\end{block}

# Rcpp functions to fit hierarchical Species Distribution Models
## Models definition
...
library(knitr)
load("~/presentation_useR/presentation_useR/draws.RData")
...
```{r time}
load("~/presentation_useR/presentation_useR/jSDM_boral_sim.RData")
load("~/presentation_useR/presentation_useR/jSDM_boral_mosquito.RData")
load("~/presentation_useR/presentation_useR/jSDM_boral_eucalypts.RData")
load("~/presentation_useR/presentation_useR/jSDM_boral_frogs.RData")
load("~/presentation_useR/presentation_useR/boral_fungi.RData")
load("~/presentation_useR/presentation_useR/jSDM_fungi.RData")
