...
On détermine une distribution *a priori* pour chacun des paramètres du modèle afin de pouvoir utiliser une méthodes d’inférence bayésienne: **l'échantillonnage de Gibbs** permettant d'obtenir une réalisation de $\Theta=(\theta_1,\ldots,\theta_n)$ selon la distribution jointe *a posteriori* $p(\theta_1,\ldots,\theta_n|x)$.  
...
### Méthodes d’inférence selon la fonction de lien choisie
...
\bcols
\bcol{0.6\textwidth}
\includegraphics[width=1.0\textwidth]{figs/jSDM-Website-09-22.png}
\ecol
\bcol{0.4\textwidth}
...
La matrice des corrélation résiduelle $R=(R_{ij})^{i=1,\ldots, J}_{j=1,\ldots, J}$ calculée de la même manière que dans l'article @Warton2015 : 
 ...
 \lambda_i .\lambda_j^T & \text{ if } i \neq j \\
  \lambda_i .\lambda_j^T & \text{ if } i=j
